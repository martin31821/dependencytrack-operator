...
```sh
make docker-build docker-push IMG=<some-registry>/deptrack-operator:tag
```
...
```sh
make deploy IMG=<some-registry>/deptrack-operator:tag
```
...
```sh
make build-installer IMG=<some-registry>/deptrack-operator:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/deptrack-operator/<tag or branch>/dist/install.yaml
```
