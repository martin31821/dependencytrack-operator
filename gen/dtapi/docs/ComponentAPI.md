...
[**GetOccurrences**](ComponentAPI.md#GetOccurrences) | **Get** /v1/component/{uuid}/occurrence | Returns the occurrences of a component
...
func main() {
	...
	component := *openapiclient.NewComponent("Classifier_example", "Name_example", *openapiclient.NewProject(int64(123), "Name_example", "Uuid_example"), "Uuid_example") // Component |  (optional)
...
> []ComponentOccurrence GetOccurrences(ctx, uuid).PageNumber(pageNumber).PageSize(pageSize).Offset(offset).Limit(limit).SortName(sortName).SortOrder(sortOrder).Execute()

Returns the occurrences of a component
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ComponentAPI.GetOccurrences(context.Background(), uuid).PageNumber(pageNumber).PageSize(pageSize).Offset(offset).Limit(limit).SortName(sortName).SortOrder(sortOrder).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ComponentAPI.GetOccurrences``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetOccurrences`: []ComponentOccurrence
	fmt.Fprintf(os.Stdout, "Response from `ComponentAPI.GetOccurrences`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetOccurrencesRequest struct via the builder pattern
...
[**[]ComponentOccurrence**](ComponentOccurrence.md)
