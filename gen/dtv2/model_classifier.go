...
const (
	...
	OPERATING_SYSTEM       Classifier = "OPERATING_SYSTEM"
	DEVICE                 Classifier = "DEVICE"
	FIRMWARE               Classifier = "FIRMWARE"
	FILE                   Classifier = "FILE"
	PLATFORM               Classifier = "PLATFORM"
	DEVICE_DRIVER          Classifier = "DEVICE_DRIVER"
	MACHINE_LEARNING_MODEL Classifier = "MACHINE_LEARNING_MODEL"
	DATA                   Classifier = "DATA"
	CRYPTOGRAPHIC_ASSET    Classifier = "CRYPTOGRAPHIC_ASSET"
)

// All allowed values of Classifier enum
var AllowedClassifierEnumValues = []Classifier{
	"APPLICATION",
	"FRAMEWORK",
	"LIBRARY",
	"CONTAINER",
...
func (v *Classifier) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := Classifier(value)
	for _, existing := range AllowedClassifierEnumValues {
	...
	return fmt.Errorf("%+v is not a valid Classifier", value)
}

// NewClassifierFromValue returns a pointer to a valid Classifier
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewClassifierFromValue(v string) (*Classifier, error) {
	ev := Classifier(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for Classifier: valid values are %v", v, AllowedClassifierEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v Classifier) IsValid() bool {
	for _, existing := range AllowedClassifierEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to classifier value
func (v Classifier) Ptr() *Classifier {
	return &v
}
