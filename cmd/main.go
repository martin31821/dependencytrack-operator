...
func init() {
	...
	utilruntime.Must(dependencytrackv1alpha1.AddToScheme(scheme))
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "e939f848.mko.dev",
	...
	if namespace == "" {
		setupLog.Error(fmt.Errorf("POD_NAMESPACE env var not set"), "unable to determine operator namespace")
		os.Exit(1)
	}

	deptrackURL := os.Getenv("DEPTRACK_URL")
	credentialsSecret := os.Getenv("DEPTRACK_CREDENTIALS_SECRET")
	if deptrackURL == "" || credentialsSecret == "" {
		setupLog.Error(fmt.Errorf("missing required env vars"), "DEPTRACK_URL and DEPTRACK_CREDENTIALS_SECRET must be set")
		os.Exit(1)
	}
